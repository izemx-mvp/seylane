...
import {
  ...
  Linkedin, Instagram, Globe, Upload, ImageIcon, Check,
} from "lucide-react";
import { cn } from "@/lib/utils";
import type { Idea, Platform } from "@/lib/mock-data";
...
const PLATFORM_META: Record<Platform, { name: string; icon: React.ComponentType<{ className?: string }>; color: string }> = {
  linkedin:  { name: "LinkedIn",  icon: Linkedin,  color: "text-[#0A66C2]" },
  instagram: { name: "Instagram", icon: Instagram, color: "text-pink-500" },
  website:   { name: "Site web",  icon: Globe,     color: "text-primary" },
};
...
function IdeasTab() {
  ...
  const generate = () => {
    ...
    setTimeout(() => {
      ...
      const imgs = ["1521737604893-d14cc237f11d","1497215728101-856f4ea42174","1552664730-d307ca884978","1519389950473-47ba0277781c","1573496359142-b8d87734a5a2"];
      const newIdeas: Idea[] = Array.from({ length: count }, (_, i) => ({
        ...
        media: `https://images.unsplash.com/photo-${imgs[i % imgs.length]}?auto=format&fit=crop&w=800&q=80`,
        status: "draft",
      }));
  ...
  const ideasImg = state.ideas.filter((i) => i.format === "image");
    ...
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{ideasImg.length} idées · visuels IA générés depuis votre configuration</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="gap-2"><Wand2 className="h-4 w-4" /> Générer des idées</Button></DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground border-l-2 border-gold pl-3">L'IA génère des visuels image adaptés à votre ton et vos objectifs.</p>
            </div>
            <DialogFooter>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {ideasImg.map((idea) => <IdeaCard key={idea.id} idea={idea} onOpen={() => setDetail(idea)} />)}
      </div>
...
function IdeaCard({ idea, onOpen }: { idea: Idea; onOpen: () => void }) {
  ...
  const dup = () => { set("ideas", [{ ...idea, id: `idea-${Date.now()}` }, ...state.ideas]); toast.success("Idée dupliquée"); };
  ...
  return (
    <Card className="overflow-hidden group hover-lift hover:-translate-y-1 hover:shadow-xl transition-all">
      <div className="relative aspect-video bg-muted cursor-pointer" onClick={onOpen}>
        ...
        <Badge className="absolute top-2 left-2 bg-primary text-primary-foreground gap-1"><ImageIcon className="h-3 w-3" /> Image</Badge>
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center gap-1 mt-3">
          {idea.platforms.map((p) => { const Icon = PLATFORM_META[p].icon; return <Icon key={p} className={cn("h-3.5 w-3.5", PLATFORM_META[p].color)} />; })}
        ...
        <div className="flex items-center gap-1 mt-3 border-t pt-3">
          <Button size="sm" variant="ghost" onClick={onOpen}><Edit3 className="h-3.5 w-3.5" /></Button>
          <Button size="sm" variant="ghost" onClick={dup}><Copy className="h-3.5 w-3.5" /></Button>
          <AlertDialog>
...
function IdeaSheet({ idea, onClose }: { idea: Idea | null; onClose: () => void }) {
  ...
  useMemo(() => {
    if (idea) {
      setCaption(idea.caption); setPlatforms(idea.platforms);
  ...
  const save = (status: Idea["status"]) => {
    ...
    set("ideas", state.ideas.map((i) => i.id === idea.id ? { ...i, caption, platforms, status, scheduledFor: status === "draft" ? undefined : d.toISOString() } : i));
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent className="w-[560px] sm:max-w-none overflow-y-auto">
        <SheetHeader><SheetTitle className="font-display tracking-tight">{idea.title}</SheetTitle></SheetHeader>
        <div className="mt-5 space-y-4">
          <div className="relative rounded-xl overflow-hidden aspect-video bg-muted">
            <img src={idea.media} alt="" className="w-full h-full object-cover" />
            <Badge className="absolute top-3 left-3 bg-black/70 text-white"><ImageIcon className="h-3 w-3 mr-1" /> Image IA</Badge>
          </div>
          <div>
          ...
          <Button variant="outline" className="w-full" onClick={() => toast.success("Visuel régénéré")}><RefreshCw className="h-4 w-4 mr-2" /> Régénérer l'image</Button>
          <div className="grid grid-cols-2 gap-2 pt-2 border-t">
            <Button variant="outline" onClick={() => save("scheduled")}><CalendarDays className="h-4 w-4 mr-2" /> Planifier</Button>
            <Button onClick={() => save("published")}><Send className="h-4 w-4 mr-2" /> Publier maintenant</Button>
          </div>
        </div>
      </SheetContent>
    </Sheet>
                  ...
                  <div className="space-y-1">
                    {cell.events.map((e) => (
                      <button key={e.id} onClick={() => setSelected(e)}
                        className={cn("w-full text-left truncate text-[10px] px-1.5 py-1 rounded flex items-center gap-1 hover:opacity-80",
                          e.status === "published" ? "bg-success/15 text-success" : "bg-primary/10 text-primary")}>
                        <ImageIcon className="h-2.5 w-2.5 shrink-0" /> {e.title}
                      </button>
                    ))}
                  </div>
                </>
...
function CreatePostButton() {
  ...
  const reset = () => { setStep(1); setTitle(""); setPrompt(""); setPlatforms(["linkedin"]); setPreview(null); setCaption(""); setMode("now"); };

  const goPreview = () => {
    setGenerating(true);
    setTimeout(() => {
      setPreview("https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=1000&q=80");
  ...
  const publish = () => {
    ...
    const idea: Idea = {
      ...
      hashtags: ["#Seylane", "#PeopleManagement"], format: "image", platforms,
      suggestedAt: new Date().toISOString(), score: 88,
      media: preview || "https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=1000&q=80",
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset(); }}>
      ...
      <DialogContent className="max-w-2xl">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div><Label>Titre / angle</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex : Success story CFO placé en 6 semaines" /></div>
            <div><Label>Prompt / description</Label><Textarea rows={4} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Décrivez le message et l'ambiance visuelle souhaitée…" /></div>
            <div>
            ...
            <div className="text-xs text-muted-foreground border-l-2 border-gold pl-3">
              Tonalité prise depuis la configuration de chaque plateforme sélectionnée.
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="relative rounded-xl overflow-hidden aspect-video bg-muted">
              ...
              <Button size="sm" variant="secondary" className="absolute bottom-3 right-3" onClick={goPreview}><RefreshCw className="h-3 w-3 mr-1" /> Régénérer</Button>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                <button type="button" onClick={() => setMode("now")}
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">Diffusion immédiate sur les plateformes sélectionnées.</div>
                </button>
                <button type="button" onClick={() => setMode("schedule")}
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">Choisissez la date et l'heure.</div>
                </button>
              </div>
            </div>
            {mode === "schedule" && (
